...
**ODMN-inspired revenue regressor (`src/models/revenue_regressor.py`).** Three LightGBM boosters—one per horizon (D1, D7, D14)—train exclusively on confirmed buyers and share order-preserving losses. Custom RMSLE/MAE callbacks reward smooth targets, while regularization penalties deter overestimation. HHISTUS prunes flat zero-revenue sections so each booster concentrates on informative spenders without sacrificing whale recall.

**Ensemble meta-learner (`src/models/ensemble.py`).** An ElasticNet regressor, a RandomForestRegressor, and an XGBoost regressor act as base learners, and a LightGBM meta learner performs the final blend to predict unconditional D7 revenue. The meta-feature set includes buyer probability, per-horizon LightGBM outputs, whale gate signals, device/geo risk factors, and engineered caps that prevent any single component from dominating. The final prediction multiplies the buyer probability curve against the revenue expectation, optionally applying calibration for submission alignment.
